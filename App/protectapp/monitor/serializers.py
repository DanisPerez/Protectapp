...
class DispositivoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Dispositivo
        fields = ['id', 'usuario', 'nombre', 'modelo', 'estado', 'eventos', 'estado_texto', 'ultima_actividad', 'unique_id']
    
    # Asegurar que nombre y modelo sean obligatorios
    ...
    def validate_modelo(self, value):
        if not value:
            raise serializers.ValidationError("El modelo del dispositivo es obligatorio.")
        return value
...
class LlamadasSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # Obtener el dispositivo_id del contexto (agregado en la vista)
        dispositivo_id = self.context.get('dispositivo_id')
        validated_data['dispositivo_id'] = dispositivo_id
...
class MensajesSerializer(serializers.ModelSerializer):
    class Meta:
        model = Mensajes
        fields = ['dispositivo', 'numero', 'contenido', 'fecha', 'hora', 'tipo'] 

# Serializer para los Contactos
class ContactosSerializer(serializers.ModelSerializer):
    class Meta:
        model = Contactos
        fields = ['dispositivo', 'nombre', 'numero']
...
class VerificacionPermisosSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'permiso', 'estado_permiso', 'fecha_verificacion', 'critico']
